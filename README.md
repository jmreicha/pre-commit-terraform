...
1. `terraform_test` supports custom arguments for filtering filenames to run tests against and filtering paths to target specific modules/directories. Passing no args tells the hook to run all tests.
    ...
    - id: terraform_test
      args:
        # First argument filters the test files to run
        - --args=unit
        # Second argument filters the modules to run
        - --arg=-filter=modules/my-module
    ```

2. `terraform_test` only runs per repository.

> [!NOTE]
> Terraform 1.6 or above is required for this hook to work.
